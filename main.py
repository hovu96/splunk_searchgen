...
def main():
    query = os.getenv("SEARCH_GEN_SPL")
    ...
    searches_executed = 0
    target_searches_per_second = float(os.getenv("SEARCH_GEN_SPS", 5.0))
    spawn_counter = int(target_searches_per_second)

    while True:
        for _ in range(spawn_counter):
            searches_executed += 1
            job = splunk.search(query)
            jobs[job.name] = job
        ...
        for job_name in list(jobs.keys()):
            ...
            if job.is_ready():
                ...
                for result in splunklib.results.ResultsReader(results):
                    ...
                    result_count += 1
                logging.debug("job %s finished with %s results",
        ...
        if seconds_elapsed > 60:
            logging.info("resetting measure time range")
            start_time = time.time()
            searches_executed = 0
        else:
            ...
            if searches_per_second_so_far > target_searches_per_second:
                if spawn_counter > 0:
                    spawn_counter -= 1
                    logging.info("decreased spawn counter to %s" %
                                 spawn_counter)
            else:
                spawn_counter += 1
